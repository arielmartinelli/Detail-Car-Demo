...
import { Car, Truck, Check, Sparkles, Clock, MessageCircle, ShieldCheck, ArrowRight, DollarSign } from 'lucide-react';
...
const VEHICLE_OPTIONS: VehicleTypeOption[] = [
  { id: 'hatchback', name: 'Hatchback / Compacto', example: 'VW Golf, Peugeot 208, Polo, Onix', iconName: 'Car' },
  { id: 'sedan', name: 'Sedán / Coupé', example: 'VW Vento, Audi A4, BMW Serie 3, Corolla', iconName: 'Car' },
  { id: 'suv', name: 'SUV / Crossover', example: 'VW Taos, Jeep Compass, SW4, Bronco', iconName: 'Truck' },
  { id: 'pickup', name: 'Pickup / 4x4', example: 'Toyota Hilux, VW Amarok, Ford Ranger, RAM', iconName: 'Truck' },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="mt-4 text-slate-400 text-sm sm:text-base">
            Seleccioná la categoría de tu vehículo y los tratamientos que deseas. Calculamos el presupuesto estimado al instante con opción de reserva directa.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-7 flex flex-col gap-8">
            ...
            {/* Step 1: Vehicle Type */}
            <div className="glass-panel p-6 rounded-2xl border border-purple-900/40">
              <h3 className="text-sm font-bold text-purple-300 uppercase tracking-wider mb-4 flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-purple-600/30 text-purple-300 text-xs flex items-center justify-center border border-purple-500/40">1</span>
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 rounded-xl text-left transition-all duration-300 flex flex-col justify-between border ${
                        isSelected
                          ? 'bg-purple-950/70 border-purple-500/80 shadow-lg shadow-purple-950/50 ring-1 ring-purple-500/50'
                          : 'bg-[#0d0e16]/60 border-purple-950/40 hover:border-purple-800/40 hover:bg-purple-950/20'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-white text-sm">{v.name}</span>
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center ${isSelected ? 'bg-purple-500 text-white' : 'border border-slate-600'}`}>
                          {isSelected && <Check className="w-3 h-3" />}
                        </div>
                      </div>
                      <span className="text-xs text-slate-400 mt-2 font-mono">{v.example}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="glass-panel p-6 rounded-2xl border border-purple-900/40">
              <h3 className="text-sm font-bold text-purple-300 uppercase tracking-wider mb-4 flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-purple-600/30 text-purple-300 text-xs flex items-center justify-center border border-purple-500/40">2</span>
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-xl cursor-pointer transition-all duration-200 border flex items-center justify-between ${
                        isChecked
                          ? 'bg-purple-950/60 border-purple-500/60 shadow-md shadow-purple-950/30'
                          : 'bg-[#0d0e16]/40 border-purple-950/30 hover:border-purple-900/50'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-6 h-6 rounded-lg flex items-center justify-center transition-colors ${isChecked ? 'bg-purple-600 text-white' : 'border border-slate-700 bg-slate-900'}`}>
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400 mt-0.5 line-clamp-1">{service.subtitle}</p>
                        </div>
                      </div>

                      <div className="text-right pl-2">
                        <span className="text-sm font-extrabold text-purple-300">{formattedServicePrice}</span>
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-28">
            <div className="glass-panel p-6 rounded-3xl border border-purple-500/40 shadow-2xl neon-purple-glow flex flex-col justify-between">
              ...
              <div>
                ...
                <div className="mt-4 p-3 rounded-xl bg-[#090a10] border border-purple-950/60 flex items-center justify-between">
                  <span className="text-xs text-slate-400">Tipo de Vehículo:</span>
                  <span className="text-xs font-bold text-white">{selectedVehicleObj?.name}</span>
                </div>

                {/* Selected services items */}
                <div className="mt-4 flex flex-col gap-2 max-h-[220px] overflow-y-auto pr-1">
                  {selectedServices.map((s) => (
                    <div key={s.id} className="flex items-center justify-between text-xs py-1.5 border-b border-purple-950/30">
                ...
                {/* Benefits checklist */}
                <div className="mt-6 pt-4 border-t border-purple-950/50 flex flex-col gap-2">
              ...
              <div className="mt-8 pt-6 border-t border-purple-900/50">
                <div className="flex items-end justify-between mb-6">
                  ...
                  <span className="text-[11px] text-slate-500 font-mono">* Sujeto a inspección de laca</span>
                </div>

                <button
                  onClick={handleBooking}
                  className="w-full flex items-center justify-center gap-3 py-4 px-6 rounded-xl font-bold text-white text-sm bg-gradient-to-r from-purple-600 via-purple-500 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 shadow-xl shadow-purple-900/60 hover:shadow-purple-600/60 transition-all duration-300 transform hover:-translate-y-1 active:translate-y-0"
                >
